...
function attempt_xfer(term, password) {
    ...
    if (src_account == undefined) {
        print_error(term, "Le compte source n'est pas défini");
    } else if (dest_account == undefined) {
        print_error(term, "Le compte de destination n'est pas défini");
    } else if (user == undefined) {
        print_error(term, "AL'utilisateur autorisé n'est pas défini");
    } else if (amount == undefined) {
        print_error(term, "Le montant n'est pas défini");
    } else {
        term.pause();
        term.echo(" ");
        print_info(term, "Tentative de transfert $" + amount.toString() + " '" + src_account + "' -> '" + dest_account + "'");
        args = {
        ...
        $.post('/federal_reserve/json/xfer', args, function(results) {
            term.echo(" ");
            if (results == null) {
                print_error(term, "ERREUR: Réponse mal formée du serveur");
            } else {
                if ('success' in results) {
                    term.echo("[[b;#00FF00;]-$-] " + results['success']);
                } else if ('error' in results) {
                    print_error(term, "ERREUR: " + results['error']);
                } else {
                    print_error(term, "ERREUR: Réponse inconnue du serveur");
                }
            }
            term.echo(" ");
            term.resume();
...
function greetings(term) {
    ...
    function display(term, index) {
        ...
        if (localStorage["intro"] == "metasploit") {
            ...
            print_info(term, "Démarrage de la console ...");
            term.resume();
        }
    }
    display(term, index);
...
$(document).ready(function() {
    $('#console').terminal({
            users: function() {
                term = this;
                term.echo(" ");
                print_info(term, "Consultation de la base de données, veuillez patienter ...");
                term.pause();
                $.getJSON('/federal_reserve/json/ls?data=users', function(data) {
                    if ('users' in data) {
                        ...
                        print_error(term, "ERREUR: Échec de la demande.")
                    }
                    term.resume();
                });
            },
            ...
            transfer: function() {
                ...
                term.push(function(src_account) {
                    ...
                    prompt: "[[;#990066;#000]-?-] Nom du compte source (transfer from): ",
                });
                term.push(function(dest_account) {
                    localStorage.setItem("dest_account", dest_account);
                    term.pop();
                }, {
                    prompt: "[[;#990066;#000]-?-] Nom du compte de destination (transfer to): ",
                });
            },
            ...
            help: function() {
                this.echo(" ");
                this.echo(" [[b;;]accounts] - Obtenir une liste des comptes bancaires actifs");
                this.echo(" [[b;;]users]    - Obtenir des informations sur les utilisateurs");
                this.echo(" [[b;;]transfer] - Transfert de fonds d'un compte à l'autre");
                this.echo(" [[b;;]intro]    - Modifier l'animation d'intro de la console");
                this.echo(" ");
            }
        }, {
            prompt: get_prompt(),
